...
# #                    h = F.interpolate(h, size=h_skip.shape[2:], mode='nearest')  # 上采样对齐空间尺寸
# #                 h = self.up[i_level].block[i_block](
# #                    torch.cat([h, h_skip], dim=1), temb)
...
import torch.nn as nn
import torch.nn.functional
# from models.hwab import HWAB  # 根据实际模块路径调整
# This script is from the following repositories
# https://github.com/ermongroup/ddim
# https://github.com/bahjat-kawar/ddrm
...
class DiffusionUNet(nn.Module):
    def __init__(self, config):
        ...
        self.conv_out = torch.nn.Conv2d(block_in,
                                        ...
                                        padding=1)
        # 在UNet的初始化部分添加
        # self.hwab = HWAB(n_feat=block_in, o_feat=block_in).to(config.device)  # 与block_in通道匹配
    def forward(self, x, t):
        # assert x.shape[2] == x.shape[3] == self.resolution
        # 记录原始输入尺寸
        ...
        h = self.norm_out(h)
        h = nonlinearity(h)
        # h = self.hwab(h)  # 新增的HWAB处理
        h = self.conv_out(h)
        # h = h[:, :, :input_height, :input_width]  # 根据原始输入尺寸裁剪
        return h