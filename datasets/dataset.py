...
class LLdataset:
    ...
    def get_loaders(self):
        train_dataset = AllWeatherDataset(self.config.data.data_dir,
                                          ...
                                          filelist='our485/{}_train.txt'.format(self.config.data.train_dataset))
        val_dataset = AllWeatherDataset(self.config.data.data_dir,
                                        patch_size=self.config.data.patch_size,
                                        filelist='eval15/{}_val.txt'.format(self.config.data.val_dataset), train=False)
...
class AllWeatherDataset(torch.utils.data.Dataset):
    ...
    def get_images(self, index):
        ...
        low_img_name_path = os.path.join(self.dir, self.data, "low/",low_img_name)
        high_img_name_path = os.path.join(self.dir, self.data,"high/", high_img_name)
        img_id = low_img_name.split('/')[-1]
        # print(low_img_name_path)
        low_img, high_img = Image.open(low_img_name_path), Image.open(high_img_name_path)
